...
class TemplateFile {
    ...
    async copy(projectPath, outputPath) {
        return fs.copy(path_1.join(Paths_1.SelfPath, 'templates', this.template, outputPath), path_1.join(projectPath, outputPath), {
            recursive: true,
        });
    }
    async evaluate(projectPath, filePath, evaluator) {
        if (this.shouldBeEvaluated) {
            return evaluator.compileFileAsync(path_1.join(projectPath, filePath));
        }
...
class UserFile {
    ...
    copy(projectPath, outputPath) {
        return fs.copy(this.userFilePath, path_1.join(projectPath, outputPath), {
            recursive: true,
        });
    }
